 ...
 *   Una cometa appare ogni ~20 secondi.
 */
...
function resetParticle(pos, vel, sizes, i) {
    // Posizione iniziale: vicino al sole
...
export function updateSolarWind(wind) {
    const { positions, velocities, sizes, geo } = wind;
...
export function createCometSystem(scene) {
    ...
    function spawnComet() {
        // Punto di partenza casuale ai margini della scena
        ...
        // Direzione verso il centro (con deviazione)
        const target = new THREE.Vector3(
            (Math.random() - 0.5) * 30,
            (Math.random() - 0.5) * 10,
            (Math.random() - 0.5) * 30
        );
        const dir = target.sub(startPos).normalize().multiplyScalar(0.6 + Math.random() * 0.4);
    ...
    function update() {
        ...
        if (frameCount >= nextSpawn) {
            spawnComet();
            nextSpawn = frameCount + 500 + Math.random() * 800;
        }
